...
public class CircularArrayWeigh {
     ...
     *
     * dx,dy:element spacings radius, of the circle Numx, numbers of rows in x
        ...
        if (Numx % 2 == 0) { //if the number of rows in x in the array is even
            for (int i = 1; i <= (Numx / 2); i++) {
                ...
                for (int j = 1; j <= Numy[(Numx / 2) + i - 1]; j++) {
                    jn = (Numy[(Numx / 2) + i - 1]);
                    if (Numy[(Numx / 2) + i - 1] % 2 == 0) { //if the number of elements in y in column Num[(Numx / 2) + i - 1] is even
                        dyj2 = (dy * (j - (jn / 2 + .5))) * (dy * (j - (jn / 2 + .5)));
                        dis = Math.pow((dxi2 + dyj2), 0.5) / radius;
                        int aux = (N / 2) + in + j - 1;
                        a[aux] = A + B * Math.cos(dis * (Math.PI / 2));
//apply the formula of the algorithm to calculate the weigths 
                        fase[aux] = .0;
                    }
...
//apply the formula of the algorithm to calculate the weigths
                    ...
                    if (Numy[(Numx / 2) + i - 1] % 2 != 0) { //if the number of elements in y in column Num[(Numx/2)+i-1] is odd
                        dyj2 = (dy * (j - (jn / 2 + 1))) * (dy * (j - (jn / 2 + 1)));
                        dis = Math.pow((dxi2 + dyj2), 0.5) / radius;
                        int aux = ((N - Numy[Numx / 2]) / 2) + in + j - 1;
                        a[aux] = A + B * Math.cos(dis * (Math.PI / 2));
