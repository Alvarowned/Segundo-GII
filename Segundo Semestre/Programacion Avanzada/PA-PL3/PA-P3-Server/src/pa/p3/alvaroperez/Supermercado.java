...
public class Supermercado {
    ...
    private boolean abierto, caja1Libre, caja2Libre, paradoCarn, paradoPesc;
    ...
    private final Lock lParaPesc = new ReentrantLock(), lParaCarn = new ReentrantLock(), lEntr = new ReentrantLock(), lCarn = new ReentrantLock(), lPesc = new ReentrantLock(), lCaj1 = new ReentrantLock(), lCaj2 = new ReentrantLock(), lCierre = new ReentrantLock();
    private final Condition pararPescadero = lParaPesc.newCondition(), pararCarnicero = lParaCarn.newCondition(), llenoEntr = lEntr.newCondition(), vacioCarn = lCarn.newCondition(), termCarn = lCarn.newCondition(), vacioPesc = lPesc.newCondition(), termPesc = lPesc.newCondition(), termCaj1 = lCaj1.newCondition(), vacioCaj1 = lCaj1.newCondition(), termCaj2 = lCaj2.newCondition(), vacioCaj2 = lCaj2.newCondition(), superCerrado = lCierre.newCondition();
    ...
    public void libCarn() throws InterruptedException {
        lCarn.lock();
        this.parar();
        if (paradoCarn) {
            lParaCarn.lock();
    ...
    public void unlockCarn() {
        lParaCarn.lock();
        try {
            pararCarnicero.signalAll();
        } finally {
            lParaCarn.unlock();
        }
        this.paradoCarn = false;
    ...
    public void setParadoCarn(boolean b) {
        this.paradoCarn = b;
    }
    ...
    public void libPesc() throws InterruptedException {
        lPesc.lock();
        this.parar();
        if (paradoPesc) {
            lParaPesc.lock();
    ...
    public void unlockPesc() {
        lParaPesc.lock();
        try {
            pararPescadero.signalAll();
        } finally {
            lParaPesc.unlock();
        }
        this.paradoPesc = false;
    }

    public void setParadoPesc(boolean b) {
        this.paradoPesc = b;
    }
