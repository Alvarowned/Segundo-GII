...
unsigned sim_mmu (ssistema * S, unsigned dir_virtual, char op)
{
    ...
    dir_fisica = marco * S->tampag + desplazamiento;  //Calcular dirección física

    referenciar_pagina(S, pagina, op);       //Marcar la página como referenciada

    if(S->detallado)                         //Modo detallado
...
void referenciar_pagina (ssistema * S, int pagina, char op)
{
	...
	S->tdp[pagina].timestamp = S->reloj; //Momento de referencia a pagina
	S->reloj ++;						 //Aumento de reloj
...
void tratar_fallo_de_pagina (ssistema * S, unsigned dir_virtual)
{
	...
	if(S->listalibres != -1) {           //Si hay marcos libres
		...
		marco = S->tdm[ult].sig;         //Tomar el siguiente (el 1º)
		
		if(marco == ult)                 //Si son el mismo es que solo quedaba uno libre
			S->listalibres = -1;
		else
			S->tdm[ult].sig = S->tdm[marco].sig;   //Sino, puentear
...
int elegir_pagina_para_reemplazo (ssistema * S)
{
    ...
    for(int i = 0; i < S->nummarcos; i++) {
		if(S->tdp[S->tdm[i].pagina].timestamp < S->tdp[S->tdm[marco].pagina].timestamp)
			marco = i;
	}
    ...
    if (S->detallado)
        printf ("@ Eligiendo (por LRU) P%d de M%d para "
                "reemplazarla\n", victima, marco);
...
void mostrar_informe_reemplazo (ssistema * S)
{
	...
	printf ("Reemplazo LRU, valor del reloj: %d; timestamp mínimo: %d; timestamp máximo: " 
		"%d\n", S->reloj, S->tdp[pag].timestamp, S->reloj - 1);
}
