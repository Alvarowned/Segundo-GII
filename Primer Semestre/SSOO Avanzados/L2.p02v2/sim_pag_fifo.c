...
int elegir_pagina_para_reemplazo (ssistema * S)
{
	...
	marco = S->listaocupados;
    victima = S->tdm[marco].pagina;

    if (S->detallado)
        printf ("@ Eligiendo (por FIFO) P%d de M%d para "
                "reemplazarla\n", victima, marco);
...
{
    printf ("Lista de ocupados:\n");
	if(S->listaocupados != -1) {
		int aux = S->listaocupados;
		do {
			printf("%d ", aux);
			aux = S->tdm[aux].sig;
		} while(aux != S->listaocupados);
		printf("\n");
	} else
		printf("NINGÚN MARCO OCUPADO.");
}
